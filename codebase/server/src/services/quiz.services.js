...
const createQuiz = async (quiz) => {
  try {
    const createdQuiz = await Quizzes.create(quiz);
    if (createdQuiz) {
...
const deleteSingleQuiz = async (quizId) => {
  try {
    const quiz = await Quizzes.findByIdAndRemove(quizId);
    if (quiz) {
      return { message: quiz, success: true };
    } else {
      return { message: "wrong QuizId", success: false };
...
const addQuestionToQuiz = async (quizId, question) => {
  try {
    ...
    if (quiz != null) {
      ...
      if (updatedQuiz) {
        const newQuiz = await Quizzes.findById(updatedQuiz._id);
        if (newQuiz) {
...
const updateSingleQuestion = async (req, quizId, questionId) => {
  try {
    ...
    if (quiz != null && quiz.questions.id(questionId) != null) {
      ...
      if (newQuiz) {
        const updatedQuiz = await Quizzes.findById(newQuiz._id);
        if (updatedQuiz) {
          return {
