...
const {getLeaderboardOfQuiz,getLeaderboardOfAllUsers
      ,submitResponse,getQuizDetailsOfUser}= require('../../services/quizResponse.services')
...
router.route("/submitResponse").post(verify, async (req, res) => {
  try{
    const submittedData=await submitResponse(req.body.quizId,req.body.userId,req.body.Score,req.body);
    if(submittedData){
